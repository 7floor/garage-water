...
int Display::writeByte(int8_t wr_data)
{
	uint8_t i, count1;
...
{
	digitalWrite(Clkpin, HIGH); //send start signal to Display
	digitalWrite(Datapin, HIGH);
	digitalWrite(Datapin, LOW);
	digitalWrite(Clkpin, LOW);
}
...
// ************************** ФОРМИРУЕМ ДАННЫЕ *****************************
void Display::display(uint8_t DispData[])
{
	uint8_t SegData[4];
	for (byte i = 0; i < 4; i ++) {
...
void Display::displayByte(uint8_t DispData[])
{
	uint8_t SegData[4];
	for (byte i = 0; i < 4; i ++) {
		//if (DispData[i] == 0x7f) SegData[i] = 0x00;
...
void Display::display(uint8_t BitAddr, uint8_t DispData)
{ 
	uint8_t SegData;
	...
	SegData = digitHEX[DispData] | PointData[DispData];
	//}  
	sendByte(BitAddr, SegData);
}

void Display::displayByte(uint8_t BitAddr, uint8_t DispData)
{
	uint8_t SegData;
	//if (DispData == 0x7f) SegData = 0x00;
	//else
...
// ************************** ОТПРАВКА НА ДИСПЛЕЙ *****************************
void Display::sendByte(uint8_t BitAddr, int8_t sendData) {
	start();          //start signal sent to Display from MCU
...
void Display::sendArray(uint8_t sendData[]) {
	start();          //start signal sent to Display from MCU
	writeByte(ADDR_AUTO);//
	stop();           //
	start();          //
...
// ******************************************
void Display::displayByte(uint8_t bit0, uint8_t bit1, uint8_t bit2, uint8_t bit3) {
	uint8_t dispArray[] = {bit0, bit1, bit2, bit3};	
	displayByte(dispArray);
}
void Display::display(uint8_t bit0, uint8_t bit1, uint8_t bit2, uint8_t bit3) {
	uint8_t dispArray[] = {bit0, bit1, bit2, bit3};	
	display(dispArray);
}
...
void Display::brightness(uint8_t brightness, uint8_t SetData, uint8_t SetAddr)
{
	Cmd_SetData = SetData;
	Cmd_SetAddr = SetAddr;
...
void Display::point(boolean PointFlag)
{
    for (byte i = 0; i < 4; i ++) {
      if (PointFlag) PointData[i] = 0x80;
      else PointData[i] = 0;
    }
  update();
}

void Display::point(uint8_t BitAddr, boolean PointFlag)
{
  if (PointFlag) PointData[BitAddr] = 0x80;
  else PointData[BitAddr] = 0;
  update();
}

// ************************** ВСЯКИЕ ФУНКЦИИ *****************************
void Display::displayClock(uint8_t hrs, uint8_t mins) {
	...
	disp_time[2] = mins / 10;
	disp_time[3] = mins % 10;
	Display::display(disp_time);
}
void Display::displayClockScroll(uint8_t hrs, uint8_t mins, int delayms) {
	if (hrs > 99 || mins > 99) return;
	uint8_t disp_time[4];	
	if ((hrs / 10) == 0) disp_time[0] = 10;
	else disp_time[0] = (hrs / 10);
...
void Display::displayClockTwist(uint8_t hrs, uint8_t mins, int delayms) {
	if (hrs > 99 || mins > 99) return;
	uint8_t disp_time[4];	
	if ((hrs / 10) == 0) disp_time[0] = 10;
	else disp_time[0] = (hrs / 10);
...
void Display::displayBar(uint8_t value) {
	...
	for (uint8_t i = 0; i < s; i++) {
		data[i] = i < s - 1 || r == 0 ? 0b00110110 : 0b00110000;
	}

	displayByte(data);
}

void Display::runningString(uint8_t DispData[], byte amount, int delayMs) {
...
void Display::scroll(uint8_t DispData[], int delayms) {
	byte DispDataByte[4];
	for (byte i = 0; i < 4; i++) {
...
void Display::scroll(uint8_t bit0, uint8_t bit1, uint8_t bit2, uint8_t bit3, int delayms) {
	byte DispData[] = {digitHEX[bit0], digitHEX[bit1], digitHEX[bit2], digitHEX[bit3]};
	Display::scrollByte(DispData, delayms);
}

void Display::scroll(uint8_t BitAddr, uint8_t DispData, int delayms) {
	byte DispDataByte = digitHEX[DispData];
	scrollByte(BitAddr, DispDataByte, delayms);
}

void Display::scrollByte(uint8_t bit0, uint8_t bit1, uint8_t bit2, uint8_t bit3, int delayms) {
	byte DispData[] = {bit0, bit1, bit2, bit3};
	Display::scrollByte(DispData, delayms);
}

void Display::scrollByte(uint8_t DispData[], int delayms) {
	byte lastBytes[4];
	byte step;
	byte stepArray[4];
...
void Display::scrollByte(uint8_t BitAddr, uint8_t DispData, int delayms) {
...
void Display::twist(uint8_t BitAddr, uint8_t DispData, int delayms) {
	twistByte(BitAddr, digitHEX[DispData], delayms);
}

void Display::twist(uint8_t DispData[], int delayms) {
	byte newData[4];
	for (byte i = 0; i < 4; i++) {
...
void Display::twist(uint8_t bit0, uint8_t bit1, uint8_t bit2, uint8_t bit3, int delayms) {
	byte DispData[] = {digitHEX[bit0], digitHEX[bit1], digitHEX[bit2], digitHEX[bit3]};
	Display::twistByte(DispData, delayms);
}

void Display::twistByte(uint8_t bit0, uint8_t bit1, uint8_t bit2, uint8_t bit3, int delayms) {
	byte DispData[] = {bit0, bit1, bit2, bit3};
	Display::twistByte(DispData, delayms);
}

void Display::twistByte(uint8_t DispData[], int delayms) {
	byte step;
	byte stepArray[4];
	boolean changeByte[4] = {0, 0, 0, 0};
...
void Display::twistByte(uint8_t BitAddr, uint8_t DispData, int delayms) {
	byte oldByte = lastData[BitAddr];
	byte newByte = DispData;
	byte step = oldByte;
